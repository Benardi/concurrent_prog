...
python3 ./format_mem_profiling.py --nflows=$i --impl=golang\
	--source=${logs_path}massif_out_golang_$i.txt --dest=$dest

python3 ./format_mem_profiling.py --no-header --nflows=$i --impl=java\
	--source=${logs_path}massif_out_java_$i.txt --dest=$dest

for((n = 50; n <= 400; n = n*2)); do

  python3 ./format_mem_profiling.py --no-header --nflows=$n --impl=golang\
	  --source=${logs_path}massif_out_golang_$n.txt --dest=$dest

  python3 ./format_mem_profiling.py --no-header --nflows=$n --impl=java\
