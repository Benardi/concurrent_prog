...
* Code to generate dataset can be found at `java-code/src/ex_05/Main.java`

```{c, eval=FALSE, code = readLines(here::here("java-code/src/ex_05/Main.java"))}
...
```
##### We also see that the growth in the total execution time for the list-based collections grows linearly as the number of elements entered increases as long as the map-based collections appear to be random.
...
data %>%
  ...
  facet_wrap(~class)
```
##### In list-based collections, this impact caused by the increase in threads seems much more explicit and following near-linear growth.
...
##### In map-based collections the number of threads seems to have some impact but it does not follow a pattern, probably caused by the thread synchronization impact.

#### In general we can conclude that the number of threads impact the performance of the collections, such as CopyOnWriteArrayList that worsens as the number of threads increases and the SynchronizedRandomAccessList that seems to have an improvement when it leaves a unique one for more threads than as the number of threads increases greatly will have their performance degraded as well. Already in map-based collections the impact of the number of theads seems to be smaller and more random, probably caused by moments of locking multiple threads at the same time and synchronization.